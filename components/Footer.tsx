    ...
    <footer className="bg-primary text-white">
      <div className="max-w-6xl mx-auto px-4 py-12">
        <div className="grid md:grid-cols-4 gap-8">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <a 
                href={`tel:${process.env.NEXT_PUBLIC_PHONE || '(555) 123-4567'}`}
                className="flex items-center space-x-2 text-gray-400 hover:text-gray-100 transition-colors"
              >
                <Phone className="h-4 w-4" />
                <span>{process.env.NEXT_PUBLIC_PHONE || '(555) 123-4567'}</span>
              </a>
              <a 
                href={`mailto:${process.env.NEXT_PUBLIC_EMAIL || 'info@example.com'}`}
                className="flex items-center space-x-2 text-gray-400 hover:text-gray-100 transition-colors"
              >
                <Mail className="h-4 w-4" />
                <span>{process.env.NEXT_PUBLIC_EMAIL || 'info@example.com'}</span>
              </a>
            </div>
          </div>
        ...
        <div className="mt-12 pt-8 border-t border-gray-700 text-center text-gray-400">
          <p>© {new Date().getFullYear()} {process.env.NEXT_PUBLIC_COMPANY_NAME || 'Your Company Name'}. All rights reserved.</p>
